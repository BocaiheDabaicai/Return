...
function Index(props) {
        ...
        if (data.get('truck_number') === '') {
            ...
            carinfo.forEach(function (item) {
                ...
                if (data.get('truck_number') === item['label']) {
                    ...
                    axios.get("/selectInformation?return_time=" + data.get('order_date') + "&car_id=" + item['id'].toString() + "").then(response => {
                        ...
                        setTableData([temp1,temp2,temp3,temp4]);
                    }).catch(error => {
    ...
    const handleInformation = (event) => {
        ...
        if (data.get('good_number').length === 4) {
            ...
            console.log(datas)
            axios.post("/addInformation", datas).then(res => {
    ...
    const handleOpenModal = (event) => {
        ...
        if (data.get('truck_number') === '') {
            ...
            axios.get(`/addInformation/material?car_info=${data.get('truck_number')}`).then(response => {
    ...
    const createExcel = () => {
        if (carChoice === undefined) {
            ...
            axios.get("/selectInformation/table?division='" + infomation[1] + "'&return_time=" + data_string + "&carname=" + carChoice, {responseType: 'blob'}).then(res => {
    ...
    const createTimeRangeExcel = () => {
        ...
        if (carChoice === undefined) {
            ...
            if ((beginYear > endYear) || (beginYear === endYear && beginMonth > endMonth) || (beginYear === endYear && beginMonth === endMonth && beginDay > endDay)) {
                ...
                axios.get("/selectInformation/table?division='" + infomation[1] + "'&begin_date=" + begin_date + "&end_date=" + end_date + "&status=" + 1, {responseType: 'blob'}).then(res => {
                    const blob = new Blob([res.data]);//处理文档流
                    // console.log(res.data)
    ...
    const handleMessageClose = () => {
        axios.get("/selectInformation/part?division=" + infomation[1]).then(response => {
    ...
    const handleConfirmMaterial = () => {
        ...
        axios.get(`/addInformation/materialUpdate?chassis=${chassisCount}&&cargo=${cargoCount}&&car_info=${carname}`).then(response => {
            // console.log(typeof response.data,response.data)
            // console.log(typeof response.data[0],response.data[0])
            console.log(response.data)
            // console.log(carinfo)
                                ...
                                <div style={{display: "none"}}>
                                    <div style={{
                                        ...
                                        <h4 style={{
                                            textAlign: "left",
                                            marginLeft: '30px'
                                        }}>运送车号:{testData[0].car_info}
                                            <text style={{
                                                textAlign: "right",
                                                marginLeft: '600px'
                                            }}>退货日期:{testData[0].return_time}</text>
                                        </h4>
                                        <div style={{
                                            display:"flex",
                                            marginLeft:'auto',
                                                ...
                                                </thead>
                                                {
                                                    tableData[0].map((row, index) => {
                                                        return (
                                                            <tbody key={index}>
                                                            <tr>
                                                                <td style={{padding: '6px'}}>{row.gname}</td>
                                                                <td style={{padding: '6px'}}>{row.return_type}</td>
                                                                <td style={{padding: '6px'}}>{row.return_quantity}</td>
                                                            </tr>
                                                            </tbody>
                                                        )
                                                    })
